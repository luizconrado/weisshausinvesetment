({
    onRecordEvent : function(component, event, helper) {
        ...
        if(eventParams.changeType=='LOADED'){
            ...
            helper.callServer(component,'getLastEmailToLeadInfo',function(response){
    ...
    sendEmail:function(component, event, helper) {
        ...
        if(sendNewsletterEmail=='true' && (data.Newsletter__c===helper.AWAITING || data.Newsletter__c===helper.RESEND)){
            sendEmailTo.newsletter=true;
        }
        if(sendProductAEmail=='true' && (data.Product_A__c===helper.AWAITING || data.Product_A__c===helper.RESEND)){
            sendEmailTo.productA=true;
        }
        if(sendProductBEmail=='true'  && (data.Product_B__c===helper.AWAITING || data.Product_B__c===helper.RESEND)){
            sendEmailTo.productB=true;
        }
        if(sendProductCEmail=='true' && (data.Product_C__c===helper.AWAITING || data.Product_C__c===helper.RESEND)){
            sendEmailTo.productC=true;       
        }
        if(sendProductDEmail=='true' && (data.Product_D__c===helper.AWAITING || data.Product_D__c===helper.RESEND)){
            sendEmailTo.productD=true;
        }
        component.set('v.loader',true);
        helper.callServer(component,'sendEmailToLead',function(response){
            let state = response.getState();
            let data = response.getReturnValue();
            if (state === "SUCCESS") {
                console.log(data);
