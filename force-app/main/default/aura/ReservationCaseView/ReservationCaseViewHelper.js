({
    ...
    fetchReservations:function(component,sfBankId,filterStartDate,filterEndDate,refrenceId){
        let _helper=this;
        let selectedRecords=component.get('v.selectedReservations');
        let selectedRecordsId=selectedRecords.map(data=>data.Id);
        component.set('v.loading',true);
        _helper.callApex(component,'retriveReservations',function(response){
            ...
            if (state === "SUCCESS") {
                let records=data.map(function(rec){
                    ...
                    rec.ResolvedAt=_helper.formatDateTime(rec.Resolved_At__c);
                    rec.ExpiredAt=_helper.formatDateTime(rec.Expired_At__c);
                    rec.ExpiresAt=_helper.formatDateTime(rec.Expires_At__c);
            ...
            sfBankId:sfBankId,
            expireStartDate:filterStartDate,
            expireEndDate:filterEndDate,
