({
	onInit : function(component, event, helper) {
        ...
        let dateFilterEnd = start.getFullYear() + '-' + (start.getMonth() + 2) + '-' + start.getDate();
        ...
        helper.fetchReservations(component,bankAccountDetails.Id,new Date(dateFilterStart),new Date(dateFilterEnd),null);
    },
    getRecords:function(component, event, helper) {
        ...
        helper.fetchReservations(component,bankAccountDetails.Id,new Date(dateFilterStart),new Date(dateFilterEnd),refFilter||null);
    ...
    searchTable: function(component, event, helper) {
        ...
        if(searchText.length>2){
            ...
            for (let i = 0; i < orignalDataList.length; i++) {
                ...
                else if(booking.ResolvedAt && booking.ResolvedAt.toLowerCase().includes(searchText)) filterdList.push(booking);
                    else if(booking.ExpiredAt && booking.ExpiredAt.toLowerCase().includes(searchText)) filterdList.push(booking);
                        else if(booking.ExpiresAt && booking.ExpiresAt.toLowerCase().includes(searchText)) filterdList.push(booking);
    ...
    showRecordDetails: function(component, event, helper){
        ...
        helper.openRecordSubTab(component,recordID,'Reservation__c');
    },
    onrecordSelect: function(component, event, helper) {
        ...
        let orignalDataList=component.get('v.orignalDataList');
        let selectedBookings=component.get('v.selectedReservations');
        orignalDataList.forEach(function(data){
            if(data.Id===text){
                data.selected=checked;
            }
        });
        component.set('v.selectedReservations',orignalDataList.filter(data=>data.selected));
