({
    ...
    fetchAllRecords:function(component,sfBankId,startDate,endDate,recipentIBAN){
        let _helper=this;
        let selectedRecords=component.get('v.selectedTimedOrder');
        let selectedRecordsId=selectedRecords.map(data=>data.Id);
        component.set('v.loading',true);
        _helper.callApex(component,'retriveTimedOrders',function(response){
            ...
            if (state === "SUCCESS") {
                let records=data.map(function(rec){
                    ...
                    rec.ExecuteAt=_helper.formatDateTime(rec.Execute_At__c);
